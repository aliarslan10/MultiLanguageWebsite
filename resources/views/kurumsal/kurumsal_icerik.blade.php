...
@foreach($menu as $icerik)
	@if(($getLink) == ($icerik->sayfa_linki))
		...
		@foreach($menu as $value)
		  <li><i style="color: gray;" class="glyphicon glyphicon-chevron-right"></i> <a href='{{ asset("$value->sayfa_linki") }}'>{{$value->sayfa_adi}}</a></li>
		@endforeach
		</ul>
		@stop
		@section('kurumsal_baslik') <h2>{{$icerik->sayfa_adi}}</h2> @stop
		@section('kurumsal_yazi') @if($icerik->resim_linki) <div class="col-sm-12 resimortala"> <img width="60%" src='{{ asset("img/kurumsal/$icerik->resim_linki") }}'/> </div> @endif
